...
/**
 * GET /api/auth/google-ads/connect?projectId=xxx
 *
 * Initiates Google Ads OAuth flow for a specific project.
...
export async function GET(request: NextRequest) {
  try {
    ...
    const authUrl = getAuthorizationUrl('google_ads', state)
    ...
    return NextResponse.json(
      { error: 'Error al iniciar conexión con Google Ads' },
      { status: 500 }
    )
  }
}
