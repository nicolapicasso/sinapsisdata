...
/**
 * GET /api/auth/google-analytics/connect?projectId=xxx
 *
 * Initiates Google Analytics OAuth flow for a specific project.
...
export async function GET(request: NextRequest) {
  try {
    ...
    const authUrl = getAuthorizationUrl('google_analytics', state)
    ...
    return NextResponse.json(
      { error: 'Error al iniciar conexión con Google Analytics' },
      { status: 500 }
    )
  }
}
